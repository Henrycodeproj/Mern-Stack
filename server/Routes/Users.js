...
router.patch(
  ...
  async (req, res) => {
    ...
    if (req.results.id !== req.params.userId)
      return res
        .status(400)
        .send({ message: "You are not authorized to change this" });
    try {
      ...
      for (const socials in socialMediaLinks) {
        if (socialMediaLinks[socials])
          try {
            ...
            if (validLink)
              user.socialMedia.set(`${socials}`, socialMediaLinks[socials]);
          } catch (error) {
            user.socialMedia.set(
              `${socials}`,
              `https://${socialMediaLinks[socials]}`
            );
          }
      }
    }
  ...
  async (req, res) => {
    try {
      ...
      if (updatedResults)
        return res
          .status(200)
          .send({ new: updatedResults, prev: currentResults });
    } catch (error) {
      console.log(error);
    }
  }
