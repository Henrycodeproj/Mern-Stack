...
app.get("/verify/:token", async (req, res) => {
  try {
    ...
    if (!result)
      return res.status(404).redirect("http://localhost:3000/invalid/expired");

    const account = await UserModel.findById(result.userId._id);
    if (account.isVerified)
      return res.status(500).send("You have already been verified.");
  ...
  await newUser
    ...
    .catch((error) => {
      if (error.keyValue.username && error.code === 11000)
        res
          .status(400)
          .send(`This username ${error.keyValue.username} is already taken`);
      else if (error.keyValue.email && error.code === 11000)
        res
          .status(400)
          .send(
            `This email ${error.keyValue.email} has already been signed up.`
          );
